...
/***
 * AddressBookGUI is used to provide a GUI to the user that would enable interaction between the
 * the user and the AddressBookController.
 */
public class AddressBookGUI extends JFrame {

  /***
   * Method used to invoke later when the swingUtilities is done being initialized.
   * Creates new instances of AddressBook and AddressBookController.
   */
  ...
  public AddressBookGUI(AddressBookController controller, AddressBook addressBook) {
    ...
    saveAsItem.addActionListener(e ->
    {
      ...
      if (currentFile.exists() && JOptionPane.YES_OPTION != JOptionPane
          .showConfirmDialog(this, "Are you sure you want to overwrite this file?", "Are you sure?",
    ...
    file.add(new JSeparator());
    quitItem
        .addActionListener(al -> dispatchEvent(new WindowEvent(this, WindowEvent.WINDOW_CLOSING)));
    file.add(quitItem);
    menuBar.add(file);
    ...
    searchTextField.getDocument().addDocumentListener(new DocumentListener() {
      ...
      public void filter() {
        ...
        tableRowSorter.setRowFilter(
            RowFilter.regexFilter("(?iu)" + Pattern.quote(searchTextField.getText())));
      }
    });
    ...
    deleteButton.addActionListener(e -> { // set listener
      int selectedRow = nameList.getSelectedRow();
      if (selectedRow == -1) {
        return;
      }
    ...
    panelPanel
        .add(new JLabel("TIP: You can sort by clicking the column headers"), BorderLayout.LINE_END);
    ...
    addWindowListener(new WindowAdapter() {
      ...
      public void windowClosing(WindowEvent e) {
        ...
        if (!saveItem.isEnabled() || JOptionPane
            .showConfirmDialog(frame, "Are you sure you want to exit? Your changes will be lost.",
